...
const DEFAULT_OUT_FILE = path.join(DEFAULT_OUT_DIR, "abacus-project-chats.json");
...
async function listProjects({ headers, timeoutMs, agent }) {
  const url = "https://apps.abacus.ai/api/_listChatLLMProjects";
  const data = await requestJson(url, headers, timeoutMs, agent);
  if (!data || data.success !== true) {
    throw new Error("_listChatLLMProjects returned an error");
  }
  return Array.isArray(data.result) ? data.result : [];
}

async function listProjectConversations({ chatllmProjectId, headers, listLimit, timeoutMs, agent }) {
  const params = new URLSearchParams({
    chatllmProjectId: String(chatllmProjectId),
...
async function run() {
  ...
  console.log("═══════════════════════════════════════════════════════");
  console.log(" Abacus ChatLLM Project Exporter");
  console.log("═══════════════════════════════════════════════════════");
  ...
  const projects = await listProjects({ headers, timeoutMs, agent });
  const projectMap = new Map();
  for (const p of projects) {
    if (!p || !p.chatllmProjectId) continue;
    projectMap.set(String(p.chatllmProjectId), p);
  }

  const requestedProjectIds = args["project-id"]
    ? String(args["project-id"]).split(",").map(s => s.trim()).filter(Boolean)
    : [];
  const requestedProjectName = args["project-name"] ? normalizeProjectName(args["project-name"]) : "";

  let allowedProjectIds = new Set(projectMap.keys());
  if (requestedProjectIds.length) {
    allowedProjectIds = new Set(requestedProjectIds);
  } else if (requestedProjectName) {
    const matched = [];
    for (const [id, proj] of projectMap.entries()) {
      const name = normalizeProjectName(proj.name);
      if (name.includes(requestedProjectName)) matched.push(id);
    }
    allowedProjectIds = new Set(matched);
  }

  if (allowedProjectIds.size === 0) {
    console.error("\n❌ No matching projects found. Use --project-id or --project-name to filter.");
    process.exit(1);
  }

  const projectsForOutput = Array.from(allowedProjectIds).map(projectId => {
    const proj = projectMap.get(projectId) || {};
    return {
      chatllmProjectId: projectId,
      name: proj.name || "",
      description: proj.description ?? null,
      customInstructions: proj.customInstructions ?? null,
      accessLevel: proj.accessLevel ?? null,
  ...
  for (const projectId of allowedProjectIds) {
    const list = await listProjectConversations({
      chatllmProjectId: projectId,
      headers,
      listLimit,
      timeoutMs,
    ...
    if (list.length === listLimit) {
      console.warn(
        `⚠ Project ${projectId} returned the same count as list-limit. Re-run with a higher --list-limit if needed.`
      );
    }
  ...
  if (items.length === 0) {
    console.log("No project-linked conversations found.");
    process.exit(0);
  }
  ...
  if (shouldUseExisting && fs.existsSync(outFile)) {
    ...
    for (const c of convs) {
      existingMap.set(c.conversationId, c);
    }
  ...
  console.log(`Projects found: ${allowedProjectIds.size}`);
